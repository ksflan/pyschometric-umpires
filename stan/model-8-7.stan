...
parameters {
  ...
  // // vector[U-1] beta_umpire; // try it with this removed. THere's something funny happening with the beta_umpire parameters
  // vector[U-1] r_umpire;
...
generated quantities {
  ...
  real<lower=-1,upper=1> error[N];
  
  for(n in 1:N) {
    prob[n] = inv_logit(theta[n]);
    error[n] = call[n] - prob[n];
  }
